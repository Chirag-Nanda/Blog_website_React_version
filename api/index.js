require("dotenv").config();
const express =require("express");
const cors = require("cors");
const mongoose =require("mongoose");
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");
const cookieParser =require("cookie-parser");
const multer = require("multer");
const uploadMiddleware = multer({dest : 'uploads/'});
const fs = require("fs");
const {Schema,model}=mongoose;

const app =express();

app.use(cookieParser());
app.use('/uploads', express.static(__dirname+'/uploads'));


app.use(cors({credentials:true, origin : "http://localhost:3001"}));
app.use(express.json());


app.get('/register',function(req,res){
    res.send("Hello");
});

const userSchema = new mongoose.Schema({
    username: {type: String, unique: true, min: 5},
    password : {type: String ,unique :true}
});

const User = mongoose.model('User', userSchema);
mongoose.connect("mongodb+srv://chirag:test123@bloghog.bxp0i9l.mongodb.net/");
app.post('/register', async function(req,res){
    
    const {userName, password} = req.body;
    const salt = bcrypt.genSaltSync(10);
   

    try{
    const newUser = new User({
      username :   userName,
      password :  bcrypt.hashSync(password , salt)
    });
    
    
    const userDoc = await newUser.save();}
    catch(err){
        res.status(400).json("error");
    }
   
});

app.get('/login',function(req,res){
    res.send("Hi");
});

app.post('/login',async function(req,res){
    const {userName, password} = req.body;
    const userDoc = await User.findOne({username : userName});
    
    
    const result = bcrypt.compareSync(password, userDoc.password);

    if(result){
       jwt.sign({userName, id : userDoc._id}, process.env.SALT , {}, (err,token)=>{
         if(err){
            res.status(400).json("error");
         }
         res.cookie('token',token).json({
            id: userDoc._id,
            userName
            
         });
       });

    }
    else{
        res.status(400).json("Wrong Credentials");
    }
});



app.get('/profile', function(req, res){
  const {token} = req.cookies;
  jwt.verify(token, process.env.SALT , {},(err, info)=>{
    if(err) throw err;
    res.json(info);
  });
  
    
});

app.post('/logout', function(req,res){
  res.cookie('token', '').json('ok');
});

const PostSchema = new mongoose.Schema({
    title: String,
    summary : String,
    content: String,
    cover : String,
    author : {type: Schema.Types.ObjectId, ref:'User'},
},{
    timestamps : true,
})

const Post = mongoose.model('Post', PostSchema);

app.post('/post',uploadMiddleware.single('file'), async function(req,res){
   const {originalname ,path} = req.file;
   const parts=originalname.split('.');
   const ext = parts[parts.length-1];
   const newPath = path + '.' + ext;
   fs.renameSync(path, newPath);
   const {title, content, summary} =req.body;
   const {token} = req.cookies;
  jwt.verify(token, process.env.SALT , {}, async (err, info)=>{
    if(err) throw err;
    const newPost = new Post({
        title : title,
        content : content,
        summary : summary,
        cover : newPath,
        author : info.id,
      }); 
   
      const resDoc = await newPost.save();
      
      res.json(resDoc);
  });

   
    
});

app.get('/post', async function(req,res){
   res.json(await Post.find().populate('author',['username']).sort({createdAt: -1}).limit(20));
});

app.put('/post',uploadMiddleware.single('file'), async function(req,res){
  let newPath = null;
  if(req.file){
    const {originalname ,path} = req.file;
   const parts=originalname.split('.');
   const ext = parts[parts.length-1];
   newPath = path + '.' + ext;
   fs.renameSync(path, newPath);
  }
  
  const {token} = req.cookies;
  jwt.verify(token, process.env.SALT , {}, async (err, info)=>{
    if(err) throw err;
    const {id,title, content, summary} =req.body;
    const postDoc = await Post.findById(id);
    const isAuthor = JSON.stringify(info.id) === JSON.stringify(postDoc.author);
    if(!isAuthor){
       res.status(400).json("you are not author");
       throw "You are not the author";
    }
    
    await postDoc.updateOne(
     {title: title,
     content: content,
     summary: summary,
     cover : newPath? newPath : postDoc.cover,
     });

    res.json(postDoc);

  });

} );

app.get('/posts/:id', async(req,res)=>{
    const {id} = req.params;
     const postDoc =await Post.findById(id).populate('author',['username']);
     res.json(postDoc);
});


app.listen(3000, function(err){
    if(!err){
        console.log("Success");
    }
});